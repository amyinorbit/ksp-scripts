...
function inst_heading {
    local np is latlng(90,0).
    return wrap_heading(-mod(360 - np:bearing,360)).
}

function inst_az_capped {
    parameter inc.
    parameter tgt_vx.
    parameter margin.
    
    local hdg_min is wrap_heading(inst_heading() - margin).
    local hdg_max is wrap_heading(inst_heading() + margin).
    local ideal_az is wrap_heading(inst_az(inc, tgt_vx)).
    
    print inst_heading().
    return min(max(ideal_az, hdg_min), hdg_max).
}

function launch_az {
    declare parameter inc.
    return arcsin(cos(inc) / cos(ship:latitude)).
}
